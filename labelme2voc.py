...
#3.获取待处理文件
files = glob(f"{labelme_path}*.json")
...
for json_file_ in files:
    ...
    with codecs.open(f"{saved_path}Annotations/{json_file_}.xml", "w", "utf-8") as xml:
...
image_files = glob(f"{labelme_path}*.jpg")
print("copy image files to VOC007/JPEGImages/")
for image in image_files:
    shutil.copy(image, f"{saved_path}JPEGImages/")

#6.split files for txt
txtsavepath = f"{saved_path}ImageSets/Main/"
with open(f'{txtsavepath}/trainval.txt', 'w') as ftrainval:
    ftest = open(f'{txtsavepath}/test.txt', 'w')
    ftrain = open(f'{txtsavepath}/train.txt', 'w')
    fval = open(f'{txtsavepath}/val.txt', 'w')
